...
import { createStarterPolicy, enforcePolicy, savePolicies } from '../src/policy/engine';
...
async function demonstrateObserveMode(storage: StorageDriver) {
  ...
  const restrictivePolicy: PolicySetV1 = {
    version: 1,
    rules: [
      { effect: "deny", ops: ["invite.create"], time: { start: "18:00", end: "08:00" } } // After hours
    ]
  };
...
function demonstrateSchemaVersioning(): PolicySetV1 {
  ...
  const advancedPolicy: PolicySetV1 = {
    ...
    rules: [
      ...
      { effect: "deny", ops: ["override.create"], time: { start: "22:00", end: "06:00" } }
    ]
  };
...
function demonstrateStarterPolicy() {
  ...
  starter.rules.forEach((rule, i) => {
    console.log(`  Rule ${i + 1}: ${rule.effect} ${rule.ops?.join(', ') || 'any ops'}`);
    if (rule.targetMaxRole) console.log(`    → Max target role: ${rule.targetMaxRole}`);
    if (rule.time) console.log(`    → Time: ${rule.time.start || '00:00'} - ${rule.time.end || '23:59'} UTC`);
    if (rule.perActorDailyCap) console.log(`    → Daily cap: ${rule.perActorDailyCap}`);
  });
...
async function demonstrateProductionDeployment(storage: StorageDriver) {
  ...
  console.log('✅ Policy system ready for production with surgical enhancements');
