...
describe('Simple Policy Integration', () => {
  ...
  test('membership operations enforce daily caps', async () => {
    ...
    await expect(addMember(ownerPubKey, newUserId, 'MEMBER')).resolves.toBeUndefined();
    
    // Second addition should fail (exceeds daily cap)
    const anotherUserId = 'another-user-456';
    await expect(addMember(ownerPubKey, anotherUserId, 'MEMBER')).rejects.toThrow('POLICY_DENIED');
  });
  ...
  test('policy enforcement is optional without storage', async () => {
    ...
    await expect(addMember(ownerPubKey, ownerPubKey, 'OWNER')).resolves.toBeUndefined();
    await expect(addMember(ownerPubKey, 'user-123', 'MEMBER')).resolves.toBeUndefined();
    await expect(addMember(ownerPubKey, 'user-456', 'MEMBER')).resolves.toBeUndefined();
  });
  ...
  test('role-based restrictions work correctly', async () => {
    ...
    await expect(addMember(adminPubKey, 'new-owner-123', 'OWNER')).rejects.toThrow('POLICY_DENIED');
    
    // ADMIN should be allowed to add MEMBER
    await expect(addMember(adminPubKey, 'new-member-123', 'MEMBER')).resolves.toBeUndefined();
  });
});
