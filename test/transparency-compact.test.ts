...
import {
  ...
  emitCheckpoint,
  verifyCheckpoint,
} from '../src/transparency/compact-api';
...
describe('Transparency Log - Phase B Task 24 (Compact)', () => {
  ...
  it('2. should generate and verify inclusion proofs', async () => {
    const leaves = ['leaf0', 'leaf1', 'leaf2'].map(s =>
  ...
  it('6. should handle RETIRED signer with grace period', async () => {
    ...
    const verification2 = await verifyCheckpoint(ns, oldCheckpoint, {
      retiredGraceMs: 24 * 60 * 60 * 1000,
    });
  ...
  it('9. should handle edge cases properly', async () => {
    await expect(genProof(ns, 0, storage)).rejects.toThrow(
      'index_out_of_range'
    );
  ...
  it('10. should work with encrypted storage simulation', async () => {
    class EncryptedStorage extends MockStorage {
      ...
      private decrypt(value: string): string {
        return value.startsWith('enc:') ? atob(value.substring(4)) : value;
      }
      async setItem(key: string, value: string): Promise<void> {
