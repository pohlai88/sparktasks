...
import SpeedDial, { type SpeedDialAction, type SpeedDialProps } from '@/components/ui/SpeedDial';
...
describe('SpeedDial Component', () => {
  ...
  describe('Basic Rendering', () => {
    ...
    it('applies correct positioning classes', () => {
      ...
      expect(speedDialContainer).toHaveClass('fixed', 'top-4', 'left-4', 'custom-speed-dial');
    });
  ...
  describe('Trigger Button States', () => {
    it('renders different trigger sizes', () => {
      const { rerender } = render(<SpeedDial {...defaultProps} triggerSize="sm" />);
    ...
    it('renders different trigger variants', () => {
      const { rerender } = render(<SpeedDial {...defaultProps} triggerVariant="primary" />);
      let trigger = screen.getByRole('button');
  ...
  describe('Menu Placement', () => {
    it('renders menu in different placements', async () => {
      const user = userEvent.setup();
      const placements: Array<'top' | 'bottom' | 'left' | 'right' | 'top-left' | 'top-right' | 'bottom-left' | 'bottom-right'> = [
  ...
  describe('Accessibility', () => {
    ...
    it('marks icons as decorative', async () => {
      ...
      const iconContainers = screen.getAllByRole('menuitem').map(item => 
  ...
  describe('Animation and Styling', () => {
    ...
    it('applies staggered animation delays', async () => {
      ...
      menuItems.forEach((item) => {
        ...
        expect(item).toHaveAttribute('style', expect.stringContaining('animation-delay'));
      });
    });
