...
describe('ButtonGroup Component - Enterprise Grade', () => {
  ...
  describe('Size Inheritance', () => {
    ...
    it('allows individual button size overrides', () => {
      render(
        <ButtonGroup size="md">
          <Button size="sm" data-testid="small-btn">Small</Button>
          <Button data-testid="normal-btn">Normal</Button>
        </ButtonGroup>
      );
      
      expect(screen.getByTestId('small-btn')).toHaveAttribute('data-size', 'sm');
      expect(screen.getByTestId('normal-btn')).toHaveAttribute('data-size', 'md');
    });
    ...
    it('supports all size variants', () => {
      ...
      sizes.forEach(size => {
        ...
        expect(screen.getByTestId(`group-${size}`)).toHaveAttribute('data-size', size);
        unmount();
      });
    });
  });
  ...
  describe('Variant Inheritance', () => {
    it('applies group variant to all buttons', () => {
      ...
      expect(screen.getByTestId('btn1')).toHaveAttribute('data-variant', 'secondary');
      expect(screen.getByTestId('btn2')).toHaveAttribute('data-variant', 'secondary');
    });
    ...
    it('allows individual button variant overrides', () => {
      ...
      expect(screen.getByTestId('destructive-btn')).toHaveAttribute('data-variant', 'destructive');
      expect(screen.getByTestId('normal-btn')).toHaveAttribute('data-variant', 'secondary');
    });
    ...
    it('supports all variant types', () => {
      ...
      variants.forEach(variant => {
        ...
        expect(screen.getByTestId(`btn-${variant}`)).toHaveAttribute('data-variant', variant);
        unmount();
      });
    });
  });
    ...
    it('works with icon buttons', () => {
      render(
        <ButtonGroup>
          ...
          <Button icon={<Trash size={16} />} variant="destructive">Delete</Button>
        </ButtonGroup>
      );
  ...
  describe('Advanced Features', () => {
    it('applies custom className correctly', () => {
      ...
      expect(screen.getByTestId('custom-group')).toHaveClass('custom-group-class');
    });
  ...
  describe('Token Integration', () => {
    ...
    it('applies proper data attributes for debugging', () => {
      render(
        <ButtonGroup size="lg" variant="primary" orientation="vertical" data-testid="debug-group">
          <Button>Test</Button>
        </ButtonGroup>
      );
    ...
    it('supports complex toolbar-style grouping', () => {
      render(
        <ButtonGroup variant="outline" size="sm" aria-label="Text formatting">
          ...
          <Button icon={<Trash size={14} />} variant="destructive" aria-label="Delete document" />
        </ButtonGroup>
      );
