...
describe('Chip - Optimized Enterprise Component', () => {
  ...
  describe('Status Indicators', () => {
    it.each<ChipStatus>(['success', 'warning', 'danger', 'info'])(
    ...
    it('skips status screen reader text when aria-label provided', () => {
      render(
        <Chip status='success' ariaLabel='Custom label'>
          Success chip
        </Chip>
      );
      expect(
  ...
  describe('Interactive Features', () => {
    ...
    it('handles selection via keyboard', async () => {
      ...
      const user = userEvent.setup();
      render(
        <Chip onSelect={onSelect} interactive>
          Selectable
        </Chip>
      );
  ...
  describe('Close Functionality', () => {
    it('calls onClose when close button is clicked and label is correct', async () => {
      ...
      const user = userEvent.setup();
      render(
        <Chip onClose={onClose} ariaLabel='My chip'>
          Closable
        </Chip>
      );
    ...
    it('close button does not propagate click to parent', async () => {
      ...
      render(
        <div
          ...
          onKeyDown={() => {}}
        >
          <Chip onClose={onClose} ariaLabel='Chip'>
            Closable
          </Chip>
        </div>
      );
    ...
    it('handles close via Delete/Backspace on chip', async () => {
      ...
      const user = userEvent.setup();
      render(
        <Chip onClose={onClose} interactive>
          Closable
        </Chip>
      );
  ...
  describe('Disabled State', () => {
    it('prevents interaction when disabled', async () => {
      ...
      render(
        <Chip disabled onSelect={onSelect} onClose={onClose}>
          Disabled
        </Chip>
      );
      const chip = screen.getByTestId('chip');
  ...
  describe('Icons and Avatar', () => {
    ...
    it('displays avatar when provided', () => {
      const avatar = (
        <img data-testid='test-avatar' src='avatar.jpg' alt='User' />
      );
    ...
    it('prioritizes status icon over regular icon', () => {
      ...
      render(
        <Chip icon={icon} status='success'>
          Priority test
        </Chip>
      );
      expect(screen.queryByTestId('regular-icon')).not.toBeInTheDocument();
      expect(
        screen.getByTestId('chip').querySelector('svg')
      ).toBeInTheDocument();
    });
    ...
    it('shows both avatar and status icon', () => {
      ...
      render(
        <Chip avatar={avatar} status='success'>
          Both icons
        </Chip>
      );
      expect(screen.getByTestId('test-avatar')).toBeInTheDocument();
  ...
  describe('Counter Feature', () => {
    ...
    it('shows max counter with plus when exceeding limit', () => {
      render(
        <Chip counter={150} maxCounter={99}>
          High counter
        </Chip>
      );
      const counter = screen.getByTestId('chip-counter');
  ...
  describe('Accessibility', () => {
    it('has correct data attributes and a11y label', () => {
      render(
        <Chip variant='success' size='lg' ariaLabel='My chip'>
          Chip Content
        </Chip>
      );
    ...
    it('provides comprehensive aria attributes for interactive chips', () => {
      render(
        <Chip onSelect={vi.fn()} selected ariaLabel='Toggle chip'>
          Interactive
        </Chip>
      );
      const chip = screen.getByTestId('chip');
  ...
  describe('Prop Forwarding', () => {
    it('forwards container props correctly', () => {
      render(
        <Chip id='test-chip' data-custom='value'>
          Test
        </Chip>
      );
      const chip = screen.getByTestId('chip');
    ...
    it('forwards onClick when interactive', async () => {
      ...
      const user = userEvent.setup();
      render(
        <Chip onClick={onClick} interactive>
          Clickable
        </Chip>
      );
  ...
  describe('Edge Cases', () => {
    ...
    it('handles simultaneous interactive and close features', async () => {
      ...
      render(
        <Chip onSelect={onSelect} onClose={onClose}>
          Multi-function
        </Chip>
      );
    ...
    it('handles very long content gracefully', () => {
      const longContent =
        'This is a very long chip content that might cause layout issues if not handled properly';
