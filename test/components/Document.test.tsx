...
const mockMetadata: DocumentMetadata = {
  ...
  description:
    'Q4 financial report with detailed analytics and forecasting for the upcoming fiscal year.',
  fileSize: 2048576,
  pageCount: 25,
  isConfidential: true,
...
describe('Document Component', () => {
  ...
  describe('Basic Functionality', () => {
    ...
    it('handles click events correctly', async () => {
      ...
      const document = screen.getByRole('button', {
        name: /document: clickable document/i,
      });
    ...
    it('handles keyboard navigation', async () => {
      ...
      const document = screen.getByRole('button', {
        name: /document: keyboard document/i,
      });
      document.focus();
    ...
    formats.forEach(({ format, expectedColor }) => {
      it(`renders ${format} format with correct icon color`, () => {
        ...
        const iconElement = document.querySelector(
          `.${expectedColor.replace('-', '-')}`
        );
        if (expectedColor !== 'text-gray-600') {
          // Skip generic gray color check
    ...
    sizes.forEach(size => {
      it(`renders ${size} size correctly`, () => {
        ...
        if (size === 'detailed') {
          ...
          expect(
            screen.getByText(mockMetadata.description!)
          ).toBeInTheDocument();
  ...
  describe('Action Buttons', () => {
    ...
    it('hides preview button when showPreview is false', () => {
      render(<Document title='No Preview Test' showPreview={false} />);
    ...
    it('shows share button when showShare is true', () => {
      render(
        <Document title='Share Test' showShare={true} onShare={mockOnShare} />
      );

      const shareButton = screen.getByRole('button', {
        name: /share document/i,
      });
  ...
  describe('Special Indicators', () => {
    it('shows external link indicator when isExternal is true', () => {
      render(<Document title='External Document' isExternal={true} />);
    ...
    it('shows expiration warning for expired documents', () => {
      ...
      render(<Document title='Expired Document' metadata={expiredMetadata} />);
  ...
  describe('States and Interactions', () => {
    it('applies disabled state correctly', () => {
      ...
      const document = screen.getByRole('button', {
        name: /document: disabled document/i,
      });
    ...
    it('shows loading state correctly', () => {
      render(<Document title='Loading Document' loading={true} />);
    ...
    it('does not trigger actions when disabled', async () => {
      ...
      const document = screen.getByRole('button', {
        name: /document: disabled actions/i,
      });
      await user.click(document);
    ...
    it('does not trigger actions when loading', async () => {
      ...
      const document = screen.getByRole('button', {
        name: /document: loading actions/i,
      });
      await user.click(document);
  ...
  describe('Accessibility', () => {
    it('has proper ARIA attributes', () => {
      ...
      const document = screen.getByRole('button', {
        name: /document: accessible document/i,
      });
    ...
    it('has proper labels for action buttons', () => {
      ...
      expect(
        screen.getByRole('button', { name: /download document/i })
      ).toBeInTheDocument();
      expect(
  ...
  describe('Thumbnails and Media', () => {
    it('renders thumbnail in detailed view', () => {
      ...
      const thumbnail = screen.getByRole('img', {
        name: /preview of document with thumbnail/i,
      });
  ...
  describe('Custom Props and Styling', () => {
    ...
    it('applies custom style', () => {
      render(
        <Document title='Custom Style' style={{ backgroundColor: 'red' }} />
      );
