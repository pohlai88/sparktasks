...
import { addWitness, listWitnesses, setWitnessStatus, buildWitnessPayload, ingestWitnessSig, verifyWitnessedCheckpoint, setAuditHook, setPolicyHook, setMetricsHook } from '../src/witness';
...
const mockCheckpoint = { v: 1, ns: 'test', n: 10, rootB64u: 'abc123', at: Date.now().toString() };
const createWitness = (org: string, id: string, status: Witness['status'] = 'ACTIVE'): Witness => ({ org, id, pubB64u: `pk-${org}-${id}`, status, addedAt: new Date().toISOString() });
const createSig = (org: string, id: string): WitnessSig => ({ org, id, at: new Date().toISOString(), sigB64u: `mock-sig-${org}` });

describe('Witness Registry', () => {
  let storage: MockStorage, auditEvents: any[], policyDecisions: Record<string, boolean>;

  beforeEach(() => {
    storage = new MockStorage();
    auditEvents = [];
    policyDecisions = {};
    setAuditHook({ log: (event: string, data: any) => auditEvents.push({ event, data }) });
    setPolicyHook({ check: async (action: string, _ctx: any) => ({ allowed: policyDecisions[action] ?? true }) });
  });

  it('adds and lists witnesses', async () => {
    const w1 = createWitness('acme', 'w1'), w2 = createWitness('beta', 'w2');
  ...
  it('filters witnesses by status', async () => {
    ...
    const retired = await listWitnesses('ns1', storage, { status: ['RETIRED'] });
  ...
  it('enforces policy on witness operations', async () => {
    ...
    await expect(addWitness('ns1', createWitness('acme', 'w1'), storage)).rejects.toThrow('policy_denied');
    expect(auditEvents.find(e => e.event === 'WITNESS_ADD_DENIED')).toBeTruthy();
  });
});
...
describe('Witness Signature Ingestion', () => {
  ...
  it('ingests valid signature', async () => {
    const wc = await ingestWitnessSig('ns1', mockCheckpoint, createSig('acme', 'w1'), storage);
    
    expect(wc.base).toEqual(mockCheckpoint);
    expect(wc.sigs).toHaveLength(1);
    expect(wc.sigs[0]?.org).toBe('acme');
    expect(auditEvents.find(e => e.event === 'WITNESS_SIG_INGEST_OK')).toBeTruthy();
  });

  it('rejects unknown witness signature', async () => {
    await expect(ingestWitnessSig('ns1', mockCheckpoint, createSig('unknown', 'w1'), storage)).rejects.toThrow('unknown_witness');
    expect(auditEvents.find(e => e.event === 'WITNESS_SIG_INGEST_FAIL')).toBeTruthy();
  });

  it('accumulates multiple signatures', async () => {
    await addWitness('ns1', createWitness('beta', 'w2'), storage);
    
    let wc = await ingestWitnessSig('ns1', mockCheckpoint, createSig('acme', 'w1'), storage);
    wc = await ingestWitnessSig('ns1', mockCheckpoint, createSig('beta', 'w2'), storage);
    
    expect(wc.sigs).toHaveLength(2);
    expect(wc.sigs.map((s: WitnessSig) => s.org).sort()).toEqual(['acme', 'beta']);
  });
});
...
describe('Witness Verification', () => {
  ...
  it('enforces required organizations', async () => {
    ...
    const policyWithRequired = { min: 1, requireOrgs: ['gamma'], bannedOrgs: [] };
    
    const wc = await ingestWitnessSig('ns1', mockCheckpoint, createSig('acme', 'w1'), storage);
    const result = await verifyWitnessedCheckpoint('ns1', wc, storage, policyWithRequired);
  ...
  it('rejects banned organizations', async () => {
    ...
    const wc = await ingestWitnessSig('ns1', mockCheckpoint, createSig('acme', 'w1'), storage);
    const result = await verifyWitnessedCheckpoint('ns1', wc, storage, policyWithBanned);
...
describe('Surgical Patches - Micro-Enhancements', () => {
  ...
  beforeEach(async () => {
    ...
    setAuditHook({ log: (event: string, data: any) => auditEvents.push({ event, data }) });
    setPolicyHook({ check: async () => ({ allowed: true }) });
    setMetricsHook({ increment: (metric: string, tags?: any) => metricsEvents.push({ metric, tags }) });
    await addWitness('ns1', createWitness('acme', 'w1'), storage);
  });
  ...
  it('🛡️ Dedup Guard: ignores duplicate (org,id) signatures', async () => {
    ...
    expect(wc.sigs).toHaveLength(1);
    expect(wc.sigs[0]?.org).toBe('acme');
    expect(metricsEvents.filter(e => e.metric === 'witness.ingest.success')).toHaveLength(2);
  });
  ...
  it('🔒 Monotonicity Check: rejects mixed base/root signatures', async () => {
    ...
    await ingestWitnessSig('ns1', mockCheckpoint, createSig('acme', 'w1'), storage);
    
    // Try to ingest signature with different base
    await expect(ingestWitnessSig('ns1', differentBase, createSig('acme', 'w1'), storage))
      .rejects.toThrow('mismatched_base');
    
    expect(metricsEvents.find(e => e.metric === 'witness.ingest.monotonicity_violation')).toBeTruthy();
  });
  ...
  it('📊 Lightweight Metrics: tracks verification outcomes by reason', async () => {
    ...
    expect(result.ok).toBe(true);
    expect(metricsEvents.find(e => e.metric === 'witness.verify.success')).toBeTruthy();
    
    // Test failed verification (insufficient signatures)
    const wcPartial = await ingestWitnessSig('ns1', { ...mockCheckpoint, n: 11 }, createSig('acme', 'w1'), storage);
    result = await verifyWitnessedCheckpoint('ns1', wcPartial, storage, policy);
    expect(result.ok).toBe(false);
    
    const failureMetric = metricsEvents.find(e => e.metric === 'witness.verify.failure');
