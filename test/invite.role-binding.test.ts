...
import { createInvite, configureMembershipDependency } from '../src/invite/create';
import { acceptInvite, configureMembershipDependency as configureAcceptDependency, configureRolePolicy } from '../src/invite/accept';
import { setInviteStorage, isUsed, markUsed } from '../src/invite/registry';
import { KeyringProvider } from '../src/crypto/keyring';
import { generateKeyPair, sign, verify, exportPublicKeyB64u, importPublicKeyB64u } from '../src/crypto/ed25519';
...
class MockStorage {
  ...
  async listKeys(): Promise<string[]> { return Array.from(this.data.keys()); }
}
...
class MockMembershipApi {
  ...
  async assertPermission(actor: string, action: 'INVITE_CREATE', context?: { targetRole?: Role }): Promise<void> {
    const userRole = this.users[actor];
    
    // Basic permission check
    if (action === 'INVITE_CREATE' && !userRole) {
      throw new Error('Access denied: INVITE_CREATE requires ADMIN, user has none');
    }
    
    if (action === 'INVITE_CREATE' && userRole && !['OWNER', 'ADMIN'].includes(userRole)) {
      throw new Error(`Access denied: INVITE_CREATE requires ADMIN, user has ${userRole}`);
    }
    ...
    if (context?.targetRole && userRole) {
      ...
      if (!allowedRoles.includes(context.targetRole)) {
        throw new Error(`Access denied: ${userRole} cannot issue ${context.targetRole} invites`);
      }
    }
  }
...
describe('Invite Role Binding & Enforcement', () => {
  ...
  beforeEach(async () => {
    ...
    configureRolePolicy({ strictLegacy: false, verifyIssuerStillAuthorized: false });
  });
  ...
  describe('Issue-time Authorization', () => {
    ...
    it('MEMBER cannot issue any invites', async () => {
      ...
      await expect(createInvite({
        ...
        signerPubB64u,
        role: 'MEMBER'
      })).rejects.toThrow('Access denied: INVITE_CREATE requires ADMIN, user has MEMBER');
    });
    ...
    it('unauthenticated user cannot issue invites', async () => {
      ...
      await expect(createInvite({
        ...
        signerPubB64u,
        role: 'VIEWER'
      })).rejects.toThrow('Access denied: INVITE_CREATE requires ADMIN, user has none');
    });
  });
  ...
  describe('Legacy Invite Handling', () => {
    it('legacy invite defaults to MEMBER role', async () => {
      ...
      const inviteBundle = { v: backup.v, createdAt: backup.createdAt, deks: backup.deks };
      ...
      const sessionKey = await crypto.subtle.deriveKey({ name: 'PBKDF2', salt, iterations: iter, hash: 'SHA-256' }, keyMaterial, { name: 'AES-GCM', length: 256 }, false, ['encrypt']);
      
      const iv = crypto.getRandomValues(new Uint8Array(12));
      const plaintext = new TextEncoder().encode(JSON.stringify(inviteBundle));
      const ciphertext = await crypto.subtle.encrypt({ name: 'AES-GCM', iv, additionalData: new TextEncoder().encode(aad) }, sessionKey, plaintext);
      ...
      const signature = await sign(signingKeys.privateKey, new TextEncoder().encode(canonical).buffer);
