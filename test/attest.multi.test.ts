...
const mockPack: any = {
  ...
  events: [{ type: 'TASK_CREATED', timestamp: '2025-01-01T00:00:00Z', payload: { title: 'test', status: 'TODAY', id: '1', priority: 'P1', tags: [] } }]
};

const mockV1Attest: PackAttestV1 = {
  v: 1,
  manifest: { content: { eventsHash: 'test123', eventsCount: 1 }, bytes: 100, meta: {} },
  att: { 
    alg: 'Ed25519', 
...
describe('Multi-Sig Attestation Engine', () => {
  ...
  describe('Threshold Verification', () => {
    ...
    it('should pass when valid signatures meet minimum threshold', async () => {
      ...
      const result = await verifyPackMulti(mockPack as any, mockAttest, 'test', policy);
  ...
  describe('Legacy Support', () => {
    ...
    it('should accept legacy pubB64u signatures when allowed', async () => {
      ...
      const mockAttest: PackAttestV2 = {
        v: 2,
        sigs: [{ pubB64u: 'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA', sigB64u: 'test_sig' }]
      };
  ...
  describe('V1 Backwards Compatibility', () => {
    ...
    it('should accept v1 attestations when min <= 1', async () => {
      ...
      const result = await verifyPackMulti(mockPack as any, mockV1Attest, 'test', policy);
  ...
  describe('Signature Deduplication', () => {
    ...
    it('should deduplicate signatures by kid', async () => {
      ...
      const mockAttest: PackAttestV2 = {
        v: 2,
        sigs: [
          { kid: 'signer1', sigB64u: 'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA' },
          { kid: 'signer1', sigB64u: 'BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB' }, // Duplicate kid
          { kid: 'signer2', sigB64u: 'CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC' }
        ]
      };
  ...
  describe('Error Handling', () => {
    ...
    it('should handle signature verification failures', async () => {
      ...
      crypto.subtle.importKey = originalImport;
      expect(result.ok).toBe(false);
      expect(result.reasons?.some(r => r.startsWith('sig_verify_failed:'))).toBe(true);
    });
