...
import { configureRevSync, planRevSync, runRevSync, revokeAndQueue } from '../src/rev/prop/sync';
...
describe('Revocation Propagation Sync', () => {
  ...
  beforeEach(() => {
    ...
    transport = {
      list: vi.fn(async (ns: string) => ({
        keys: Array.from(remoteStore.keys()).filter(k => k.startsWith(`rev:${ns}:`)),
  ...
  test('configures sync correctly', () => {
    expect(() => configureRevSync(storage, NAMESPACE, transport, [ADMIN_PUB])).not.toThrow();
  });
  ...
  test('revokeAndQueue creates valid record', async () => {
    const record = await revokeAndQueue('INVITE_REVOKED', 'invite-123', 'Security breach');
  ...
  test('maintains sync state monotonicity', async () => {
    // Set initial state
    await storage.setItem(`rev:${NAMESPACE}:__sync_state__`, JSON.stringify({ since: 'cursor-100' }));
  ...
  test('applies different revocation types', async () => {
    ...
    const records: RevRecord[] = [
      {
        v: 1, id: 'r1', ts: new Date().toISOString(), type: 'INVITE_REVOKED',
        subject: 'invite-123', issuer: { pubB64u: ADMIN_PUB, sigB64u: 'sig1' }
      },
      {
        v: 1, id: 'r2', ts: new Date().toISOString(), type: 'SIGNER_REVOKED',
        subject: 'signer-456', issuer: { pubB64u: ADMIN_PUB, sigB64u: 'sig2' }
      },
      {
        v: 1, id: 'r3', ts: new Date().toISOString(), type: 'RECOVERY_REVOKED',
        subject: 'recovery-789', issuer: { pubB64u: ADMIN_PUB, sigB64u: 'sig3' }
      }
    ];
