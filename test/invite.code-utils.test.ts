...
describe('Invite Code Checksums', () => {
  ...
  it('should reject invalid checksums', () => {
    expect(() => verifyCodeChecksum('SECRET123-XX')).toThrow('Invalid code checksum');
    expect(() => verifyCodeChecksum('INVALID')).toThrow('Invalid code format');
    expect(() => verifyCodeChecksum('CODE-ZZ')).toThrow('Invalid code checksum'); // Different invalid checksum
  });
