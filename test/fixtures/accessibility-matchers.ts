...
baseExpect.extend({
  ...
  async toBeKeyboardNavigable(locator: Locator) {
    ...
    const isNavigable =
      tabIndex !== '-1' && (isInteractive || tabIndex !== null);

    return {
      message: () =>
        `Expected element to be keyboard navigable (interactive role or positive tabindex), but it was not`,
      pass: isNavigable,
  ...
  async toHaveSemanticStructure(
    locator: Locator,
    expectedStructure: AccessibilityStructure
  ) {
    ...
    return {
      message: () =>
        `Expected element to have semantic structure matching the specification`,
  ...
  async toAnnounceChangesCorrectly(locator: Locator) {
    ...
    return {
      message: () =>
        `Expected element to properly announce changes (aria-live, role=status/alert)`,
      pass: !!hasLiveRegion,
...
// Helper functions
function validateARIAHierarchy(
  node: any,
  depth = 0
  ...
  if (
    node.role === 'button' &&
    node.children?.some((child: any) => child.role === 'button')
  ) {
    return {
      valid: false,
      message:
  ...
  if (node.role === 'heading' && node.level && depth > 0) {
    ...
    if (node.level > 6) {
      return {
        valid: false,
        message: `Heading level ${node.level} exceeds maximum of 6`,
      };
    }
  }
...
function compareStructures(
  actual: any,
  expected: AccessibilityStructure
): { isMatch: boolean; differences: string[] } {
  const differences: string[] = [];

  if (actual?.role !== expected.role) {
    differences.push(
      `Role mismatch: expected "${expected.role}", got "${actual?.role}"`
    );
  }

  if (expected.name && actual?.name !== expected.name) {
    differences.push(
      `Name mismatch: expected "${expected.name}", got "${actual?.name}"`
    );
  }

  if (expected.level && actual?.level !== expected.level) {
    differences.push(
      `Level mismatch: expected ${expected.level}, got ${actual?.level}`
    );
  }
  ...
  if (expected.children && actual?.children) {
    if (expected.children.length !== actual.children.length) {
      differences.push(
        `Children count mismatch: expected ${expected.children.length}, got ${actual.children.length}`
      );
    }
  }
