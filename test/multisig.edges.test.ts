...
const P = (over = {}) => ({ min: 2, bannedKids: [], requireKids: [], retiredGraceMs: 60_000, allowLegacy: false, ...over });
...
describe('Multi-sig edge cases', () => {
  ...
  it('requireKids vs min: require must be met even if min satisfied', async () => {
    ...
    const a1: PackAttestV2 = { v: 2, sigs: [{ kid: 'k1', sigB64u: 'sig1' }, { kid: 'k2', sigB64u: 'sig2' }] };
    expect((await verifyPackMulti(pack as any, a1, ns, P({ requireKids: ['k1'] }))).ok).toBe(true);
    
    // Test case 2: Missing required kid k1 - should fail even though min=2 is met
    const a2: PackAttestV2 = { v: 2, sigs: [{ kid: 'k2', sigB64u: 'sig2' }, { kid: 'k3', sigB64u: 'sig3' }] };
    const v = await verifyPackMulti(pack as any, a2, ns, P({ requireKids: ['k1'] }));
    expect(v.ok).toBe(false);
    expect(v.reasons?.some(r => r.includes('require_kid_missing:k1'))).toBe(true);
  });

  it('dedup by kid: duplicates count once', async () => {
    const a: PackAttestV2 = { v: 2, sigs: [{ kid: 'k4', sigB64u: 'sig1' }, { kid: 'k4', sigB64u: 'sig2' }] };
  ...
  it('retired grace boundary: ==grace counts, >grace rejected', async () => {
    ...
    const v1 = await verifyPackMulti(pack as any, a, ns, P({ min: 1, retiredGraceMs: 60_000 }));
    expect(v1.ok).toBe(true); // Mock signer is always ACTIVE
    
    // Test the threshold not met case to ensure the grace logic path exists
    const v2 = await verifyPackMulti(pack as any, a, ns, P({ min: 2, retiredGraceMs: 60_000 }));
  ...
  it('mixed local + federated: meets threshold with one local + one remote; revoke remote fails', async () => {
    ...
    let v = await verifyPackMulti(pack as any, localOnlyAttest, ns, P({ min: 1, allowLegacy: false }));
    ...
    const v1 = await verifyPackMulti(pack as any, legacyOnlyAttest, ns, P({ min: 1, allowLegacy: true }));
    const v2 = await verifyPackMulti(pack as any, legacyOnlyAttest, ns, P({ min: 1, allowLegacy: false }));
  ...
  it('audit redaction: multi-verify emits audit without raw sig bytes', async () => {
    const a: PackAttestV2 = { v: 2, sigs: [{ kid: 'k7', sigB64u: 'sensitive_signature_data' }] };
    const v = await verifyPackMulti(pack as any, a, ns, P({ min: 1 }));
    expect(v.ok).toBe(true);
  ...
  it('corrupted sig bytes → sig_verify_error:<kid>', async () => {
    ...
    vi.spyOn(crypto.subtle, 'verify').mockRejectedValue(new Error('Invalid signature format'));
    
    const v = await verifyPackMulti(pack as any, corruptedAttest, ns, P({ min: 1 }));
    expect(v.ok).toBe(false);
  ...
  it('v1 attestation with min > 1 → threshold_not_met:min=2,count=1', async () => {
    ...
    const v1Attest = {
      v: 1 as const,
      manifest: { content: { eventsHash: 'test', eventsCount: 1 }, bytes: 100, meta: {} },
      att: { alg: 'Ed25519' as const, signer: 'pubkey', sig: 'signature', ts: '2025-01-01T00:00:00Z', kid: 'legacy' }
    };
    ...
    expect(v.reasons?.some(r => r.includes('threshold_not_met:min=2,count=1'))).toBe(true);
  });

  it('REVOKED local signer → revoked_kid:<kid>', async () => {
    // Mock getSigner to return a REVOKED signer
    const originalGetSigner = (await import('../src/attestation/attest.multi')).default;
    ...
    const v = await verifyPackMulti(pack as any, revokedAttest, ns, P({ min: 1 }));
