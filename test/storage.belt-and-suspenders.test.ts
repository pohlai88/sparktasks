...
import { appendEvent, configureStorage, loadEvents } from '../src/domain/task/eventlog';
...
class ValidatingStorageDriver extends SyncLocalStorageDriver {
  ...
  setItem(key: string, value: string): void {
    ...
    if (key.endsWith('.tmp') || key.includes('events') || key.includes('test-key') || key.includes('mixed-data')) {
    ...
    for (const line of lines) {
      if (line.trim()) {
        try {
          ...
          throw new Error(`Invalid event log format: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
      }
    }
  }
