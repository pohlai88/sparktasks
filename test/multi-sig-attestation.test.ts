...
import type { ThresholdPolicy, PackAttestV2 } from '../src/attestation/multi-sig-types';
...
describe('Multi-Sig Attestation', () => {
  ...
  describe('Multi-Sig Emission (attestPackMulti)', () => {
    test('happy path - multi-sig emission with 3 signers', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        const publicKey = await crypto.subtle.exportKey('raw', keyPairs[i].publicKey);
        const privateKey = await crypto.subtle.exportKey('raw', keyPairs[i].privateKey);
    ...
    test('revoked signer - emission fails with revoked key', async () => {
      ...
      const publicKey1 = await crypto.subtle.exportKey('raw', keyPairs[0].publicKey);
      const privateKey1 = await crypto.subtle.exportKey('raw', keyPairs[0].privateKey);
      ...
      const publicKey2 = await crypto.subtle.exportKey('raw', keyPairs[1].publicKey);
      const privateKey2 = await crypto.subtle.exportKey('raw', keyPairs[1].privateKey);
      ...
      await expect(attestPackMulti(ns, kids, mockPack))
        .rejects.toThrow('Cannot use revoked signer: signer1');
    });
    ...
    test('missing signer - emission fails with unknown kid', async () => {
      ...
      await expect(attestPackMulti(ns, kids, mockPack))
        .rejects.toThrow('Signer not found: unknown-signer');
    });
  });
  ...
  describe('Multi-Sig Verification (verifyPackMulti)', () => {
    ...
    test('signature tampering - verification fails with modified signature', async () => {
      ...
      const result = await verifyPackMulti(mockPack, tamperedAttest, ns, policy);
  ...
  describe('Legacy V1 Compatibility', () => {
    test('v1 attestation - passes with min=1 policy', async () => {
      // Setup signer for v1
      const publicKey = await crypto.subtle.exportKey('raw', keyPairs[0].publicKey);
      await addSigner(ns, {
        kid: 'legacy-signer',
        pubB64u: btoa(String.fromCharCode(...new Uint8Array(publicKey)))
          .replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, ''),
  ...
  describe('Legacy Public Key Path', () => {
    test('legacy pubB64u - allowed with policy flag', async () => {
      ...
      expect(result.ok).toBe(false);
      expect(result.reasons).toContain('sig_verify_failed:legacy:' + pubB64u.slice(0, 16));
    });
    ...
    test('legacy pubB64u - denied without policy flag', async () => {
      ...
      expect(result.ok).toBe(false);
      expect(result.reasons).toContain('legacy_not_allowed:' + pubB64u.slice(0, 8) + '...');
    });
  });
  ...
  describe('Retired Signers Grace Period', () => {
    test('retired within grace - verification succeeds', async () => {
      ...
      const publicKey = await crypto.subtle.exportKey('raw', keyPairs[0].publicKey);
      const privateKey = await crypto.subtle.exportKey('raw', keyPairs[0].privateKey);
    ...
    test('retired beyond grace - verification fails', async () => {
      ...
      // Create attestation
      const attest = await attestPackMulti(ns, ['old-retired-signer'], mockPack);
      ...
      expect(result.ok).toBe(false);
      expect(result.count).toBe(0);
      expect(result.reasons).toContain('retired_out_of_grace:old-retired-signer');
    });
  });
