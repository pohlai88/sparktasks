...
import { addMember, removeMember, changeRole, configureMembership } from '../src/membership/api';
...
describe('Policy Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    keyring = { 
      exportBackup: async () => ({ v: 1, createdAt: new Date().toISOString(), deks: [] }),
      importBackup: async () => {},
      rotate: async () => {}
    };
  ...
  test('invite.create enforces policy when storage provided', async () => {
    ...
    const mockMembershipApi = {
      ...
      getMembership: async () => ({
        users: { [ownerPubKey]: 'OWNER' as Role, [adminPubKey]: 'ADMIN' as Role },
        owners: [ownerPubKey]
      })
    };
    
    const { configureMembershipDependency } = await import('../src/invite/create');
