    ...
    this.walkDirectory('.', (filePath, stats) => {
      if (stats.size > MAX_FILE_SIZE_BYTES) {
        ...
        this.violations.push(
          `File too large: ${filePath} (${sizeMB}MB > ${MAX_FILE_SIZE_MB}MB)`
        );
      }
    });
  }
    ...
    this.walkDirectory('.', filePath => {
      ...
      if (dir.includes('tools') && (ext === '.js' || ext === '.ts')) {
        if (!NAMING_PATTERNS.scripts.test(fileName)) {
          this.violations.push(
            `Script naming violation: ${filePath} (should be kebab-case)`
          );
        }
      }
      ...
      if (ext === '.tsx' || ext === '.jsx') {
        if (!NAMING_PATTERNS.components.test(fileName)) {
          this.violations.push(
            `Component naming violation: ${filePath} (should be PascalCase)`
          );
        }
      }
      ...
      if (fileName.includes('.test.') || fileName.includes('.spec.')) {
        if (!NAMING_PATTERNS.tests.test(fileName)) {
          this.violations.push(
            `Test naming violation: ${filePath} (should be *.test.* or *.spec.*)`
          );
        }
      }
      ...
      if (ext === '.css' || ext === '.scss') {
        if (!NAMING_PATTERNS.styles.test(fileName)) {
          this.violations.push(
            `Style naming violation: ${filePath} (should be kebab-case)`
          );
        }
      }
      ...
      if (ext === '.json' && !CONFIG_EXCEPTIONS.includes(fileName)) {
        if (!NAMING_PATTERNS.configs.test(fileName)) {
          this.warnings.push(
            `Config naming suggestion: ${filePath} (consider kebab-case)`
          );
        }
      }
      ...
      if (ext === '.md') {
        if (!NAMING_PATTERNS.markdown.test(fileName)) {
          this.warnings.push(
            `Markdown naming suggestion: ${filePath} (consider kebab-case)`
          );
        }
      }
    });
  ...
  validateProhibitedContent() {
    ...
    for (const dir of prohibitedDirs) {
      if (fs.existsSync(dir)) {
        this.violations.push(
          `Prohibited directory found: ${dir} (use package manager instead)`
        );
      }
    }
    ...
    for (const dir of tempDirs) {
      if (fs.existsSync(dir)) {
        this.warnings.push(
          `Temporary directory found: ${dir} (consider adding to .gitignore)`
        );
      }
    }
  }
