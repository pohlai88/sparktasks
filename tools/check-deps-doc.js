...
if (!fs.existsSync(LOCKFILE_PATH)) {
  console.error(`❌ ${LOCKFILE_PATH} missing. Commit your lockfile for reproducible installs.`);
  process.exit(1);
}

// Success message
console.log(`✅ Dependencies doc check OK (${deps.length} dependencies documented)`);

// Optional: Warn about documented dependencies that no longer exist
const documentedDeps = (doc.match(/^-\s*([a-z0-9@/_-]+)/gmi) || [])
