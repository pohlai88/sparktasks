...
// ✅ NEW: SSOT Hardcoded Class Violation Check
const EXTS = new Set(['.tsx', '.ts']);
const TARGET_DIRS = [path.join(root, 'src')];

const BANNED = [
  /\b(m[trblxy]?-\d+|p[trblxy]?-\d+|gap-\d+|w-\d+|h-\d+|size-\d+)\b/,
  /\bflex\s+items-center\s+justify-between\b/,
  /\banimate-spin\b(?=.*\b(h-\d+|w-\d+|size-\d+)\b)/,
];

let violations = [];

function walkForViolations(dir) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    const p = path.join(dir, entry.name);
    if (entry.isDirectory()) walkForViolations(p);
    else if (EXTS.has(path.extname(p))) checkFileForViolations(p);
  }
}

function checkFileForViolations(file) {
  // Skip design tokens file - it's the SSOT source and should contain hardcoded Tailwind
  if (file.includes('design/tokens.ts') || file.includes('design\\tokens.ts')) return;
...
// Run SSOT hardcoded class violation check
console.log('\n🔍 Checking for SSOT violations...');
for (const dir of TARGET_DIRS) {
  if (fs.existsSync(dir)) walkForViolations(dir);
}
...
if (violations.length) {
  ...
  console.error('Run: npm run ssot:codemod to auto-fix common patterns.');
  process.exit(1);
}
