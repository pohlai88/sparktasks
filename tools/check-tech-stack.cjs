...
if (!npmrc || !npmrc.includes('engine-strict=true'))
  errors.push('.npmrc missing or lacks "engine-strict=true".');
if (mdNode && nvm && mdNode !== nvm)
  errors.push(`Mismatch: TECH_STACK.md Node (${mdNode}) != .nvmrc (${nvm}).`);
if (mdNode && !engines) errors.push('package.json "engines.node" missing.');
if (
  mdNode &&
  engines &&
  !engines.includes(mdNode.split('.').slice(0, 2).join('.'))
)
  errors.push(
    `package.json engines.node (${engines}) does not align with ${mdNode}.`
  );

// Project structure checks
if (!exists('src/shared'))
  errors.push('src/shared directory missing (required by TECH_STACK.md).');
if (!exists('test/integration'))
  errors.push('test/integration directory missing.');
