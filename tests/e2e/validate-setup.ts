...
class E2EValidator {
  ...
  private async checkFileStructure(): Promise<void> {
    ...
    const requiredFiles = [
      ...
      'e2e/data/seed-manager.ts',
      'e2e/data/storage-state.json',
      'e2e/utils/auth-utils.ts',
  ...
  private async checkConfigurations(): Promise<void> {
    ...
    if (existsSync('e2e/tsconfig.json')) {
      try {
        const tsConfig = JSON.parse(readFileSync('e2e/tsconfig.json', 'utf-8'));
  ...
  private async checkTestData(): Promise<void> {
    ...
    if (existsSync('e2e/data/test-users.ts')) {
      try {
        // Basic syntax check by requiring the file
        const content = readFileSync('e2e/data/test-users.ts', 'utf-8');
    ...
    if (existsSync('e2e/data/storage-state.json')) {
      try {
        const storageState = JSON.parse(
          readFileSync('e2e/data/storage-state.json', 'utf-8')
        );
