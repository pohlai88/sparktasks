...
export interface PackAttestV1 {
  v: 1;
  manifest: { content: { eventsHash: string; eventsCount: number }; bytes: number; meta: any };
  att: { alg: 'Ed25519'; signer: string; sig: string; ts: string; kid?: string };
}
 ...
 */
export async function attestPackMulti(ns: string, kids: string[], pack: Sparkpack): Promise<PackAttestV2> {
  ...
  for (const kid of kids) {
    ...
    if (signer.status === 'REVOKED') throw new Error(`Cannot use revoked signer: ${kid}`);
    
    const privateKey = await crypto.subtle.importKey('raw', fromB64u(signer.privB64u), { name: 'Ed25519' }, false, ['sign']);
    const signature = await crypto.subtle.sign('Ed25519', privateKey, canonicalBytes);
    sigs.push({ kid, sigB64u: toB64u(signature) });
  }
  
  await AuditApi.log('ATTEST_MULTI_EMIT', { namespace: ns, kids, count: kids.length });
  return { v: 2, sigs };
}
 ...
 */
export async function verifyPackMulti(pack: Sparkpack, attest: PackAttest, ns: string, policy: ThresholdPolicy): Promise<MultiSigResult> {
  const canonicalBytes = getCanonicalBytes(pack);
  ...
  if (attest.v === 1) {
    if (policy.min > 1) {
      ...
      await AuditApi.log('ATTEST_MULTI_VERIFY', { namespace: ns, min: policy.min, count: 0, ok: false, reasons });
    ...
    if (attest.att.kid) {
      ...
      if (!signer) return { ok: false, count: 0, reasons: [`signer_not_found:${attest.att.kid}`] };
      if (signer.status === 'REVOKED') return { ok: false, count: 0, reasons: [`revoked_kid:${attest.att.kid}`] };
      
      if (signer.status === 'RETIRED') {
        if (!policy.retiredGraceMs || !signer.retiredAt || (Date.now() - new Date(signer.retiredAt).getTime()) > policy.retiredGraceMs) {
          return { ok: false, count: 0, reasons: [`retired_out_of_grace:${attest.att.kid}`] };
        }
      }
      
      pubKey = await crypto.subtle.importKey('raw', fromB64u(signer.pubB64u), { name: 'Ed25519' }, false, ['verify']);
    } else {
      if (!policy.allowLegacy) return { ok: false, count: 0, reasons: ['legacy_not_allowed'] };
      effectiveKid = `legacy:${attest.att.signer.slice(0, 16)}`;
      pubKey = await crypto.subtle.importKey('raw', fromB64u(attest.att.signer), { name: 'Ed25519' }, false, ['verify']);
    }
    
    const isValid = await crypto.subtle.verify('Ed25519', pubKey, fromB64u(attest.att.sig), canonicalBytes);
    const result = { ok: isValid, count: isValid ? 1 : 0, ...((!isValid) && { reasons: [`sig_verify_failed:${effectiveKid}`] }) };
    
    await AuditApi.log('ATTEST_MULTI_VERIFY', { namespace: ns, min: policy.min, count: result.count, ok: result.ok });
    return result;
  }
  ...
  for (const sig of attest.sigs) {
    ...
    if (sig.kid) {
      ...
      if (!signer) { reasons.push(`signer_not_found:${sig.kid}`); continue; }
      if (signer.status === 'REVOKED') { reasons.push(`revoked_kid:${sig.kid}`); continue; }
      ...
      if (!policy.allowLegacy) { reasons.push(`legacy_not_allowed`); continue; }
      effectiveKid = `legacy:${sig.pubB64u.slice(0, 16)}`;
      try {
        pubKey = await crypto.subtle.importKey('raw', fromB64u(sig.pubB64u), { name: 'Ed25519' }, false, ['verify']);
      } catch {
    ...
    try {
      const isValid = await crypto.subtle.verify('Ed25519', pubKey, fromB64u(sig.sigB64u), canonicalBytes);
      if (isValid) {
        valid++;
  ...
  await AuditApi.log('ATTEST_MULTI_VERIFY', { namespace: ns, min: policy.min, count: valid, ok, ...(reasons.length > 0 && { reasons }) });
