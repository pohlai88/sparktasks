...
import type { PackAttestV1, PackAttestV2, ThresholdPolicy, MultiSigResult, PackAttest } from './multi-sig-types';
...
export async function attestPackMulti(
  ...
  if (!policyCheck.allowed) {
    throw new Error(`Multi-sig emission denied by policy: ${policyCheck.reason}`);
  }
...
async function verifyV1Compat(
  ...
  if (attest.att.kid) {
    ...
    if (!signer || signer.status === 'REVOKED') {
      const reason = !signer ? `signer_not_found:${attest.att.kid}` : `revoked_kid:${attest.att.kid}`;
  ...
  const result = {
    ok: isValid,
    count: isValid ? 1 : 0,
    ...((!isValid) && { reasons: [`sig_verify_failed:${effectiveKid || 'unknown'}`] })
  };
