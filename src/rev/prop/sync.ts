...
import type { RevRecord, RevType, RevTransport, RevPlan, RevResult } from './types';
...
async function getSyncState(): Promise<{ since?: string }> {
  try { const data = await storage.getItem(`rev:${namespace}:__sync_state__`); return data ? JSON.parse(data) : {}; } catch { return {}; }
}

async function setSyncState(state: { since?: string }): Promise<void> {
  await storage.setItem(`rev:${namespace}:__sync_state__`, JSON.stringify(state));
}
...
async function applyRevocation(record: RevRecord): Promise<boolean> {
  try {
    ...
    switch (record.type) {
      case 'INVITE_REVOKED': console.log(`Revoked invite: ${record.subject}`); break;
      case 'SIGNER_REVOKED': console.log(`Revoked signer: ${record.subject}`); break;
      case 'RECOVERY_REVOKED': console.log(`Revoked recovery bundle: ${record.subject}`); break;
    }
    return true;
  } catch {
    return false;
...
export async function runRevSync(plan?: RevPlan): Promise<RevResult> {
  ...
  const result: RevResult = { applied: 0, pushed: 0, errors: [], completed: false };
  ...
  try {
    ...
    for (const key of syncPlan.pullKeys) {
      try {
        ...
        result.errors.push(`Error processing ${key}: ${error instanceof Error ? error.message : 'Unknown'}`);
      }
    }
    ...
    for (let i = 0; i < outboxKeys.length; i += 100) {
      ...
      for (const outboxKey of batch) {
        try {
          ...
          const record: RevRecord = JSON.parse(data);
          await transport.put(`rev:${namespace}:r:${record.ts}:${record.id}`, data, record.ts);
          await storage.removeItem(outboxKey);
          result.pushed++;
        } catch (error) {
          result.errors.push(`Push error ${outboxKey}: ${error instanceof Error ? error.message : 'Unknown'}`);
        }
      }
    }
    ...
    if (syncPlan.since && (!currentState.since || syncPlan.since > currentState.since)) {
...
export async function revokeAndQueue(type: RevType, subject: string, reason?: string): Promise<RevRecord> {
  if (!storage) {
    throw new Error('RevSync not configured. Call configureRevSync() first.');
  }
  ...
  const recordWithoutSig = {
    v: 1 as const, id, ts, type, subject,
    ...(reason && { reason }),
  ...
  const sigBytes = new TextEncoder().encode('placeholder-sig-' + canonical.slice(0, 16));
  const sigB64u = toB64u(sigBytes.buffer);
  
  const record: RevRecord = { ...recordWithoutSig, issuer: { pubB64u: adminPubB64u, sigB64u } };
