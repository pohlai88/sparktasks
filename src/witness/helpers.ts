...
export function filterWitnessesByStatus(witnesses: Witness[], statuses?: string[]): Witness[] {
...
export function isWitnessValid(witness: Witness, atTime: string, retiredGraceMs = 0): { valid: boolean; reason?: string } {
  if (witness.status === 'REVOKED') {
    return { valid: false, reason: `witness_revoked:${witness.org}/${witness.id}` };
  }
  ...
  if (witness.status === 'RETIRED') {
    ...
    if (now - sigTime > graceMs) {
      return { valid: false, reason: `witness_expired:${witness.org}/${witness.id}` };
    }
  }
...
export function checkPolicy(validOrgs: Set<string>, policy: WitnessPolicy): { ok: boolean; reason?: string } {
  ...
  if (validOrgs.size < policy.min) {
    return { ok: false, reason: `threshold_not_met:min=${policy.min},count=${validOrgs.size}` };
  }
