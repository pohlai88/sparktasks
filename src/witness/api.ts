...
import { Witness, WitnessSig, WitnessedCheckpointV1, WitnessPolicy, WitnessVerifyResult } from './types';
import { canonicalize, buildWitnessPayload, filterWitnessesByStatus, findWitness, isWitnessValid, countValidSigs, checkPolicy } from './helpers';
...
async function loadWitnessRegistry(ns: string, storage: StorageDriver): Promise<Witness[]> {
  const data = await storage.getItem(`wtns:${ns}:reg`);
  return data ? JSON.parse(data) : [];
}

// Save witness registry to storage
async function saveWitnessRegistry(ns: string, witnesses: Witness[], storage: StorageDriver): Promise<void> {
...
export async function addWitness(ns: string, w: Witness, storage: StorageDriver): Promise<void> {
  // Policy check
  if (policyHook) {
    const policyResult = await policyHook.check('witness.add', { ns, org: w.org, id: w.id });
    if (!policyResult.allowed) {
      auditHook?.log('WITNESS_ADD_DENIED', { ns, org: w.org, id: w.id, reason: policyResult.reason });
...
export async function setWitnessStatus(ns: string, org: string, id: string, status: Witness['status'], storage: StorageDriver): Promise<void> {
  // Policy check
  if (policyHook) {
    const policyResult = await policyHook.check('witness.status', { ns, org, id, status });
    if (!policyResult.allowed) {
      auditHook?.log('WITNESS_STATUS_DENIED', { ns, org, id, status, reason: policyResult.reason });
      throw new Error('policy_denied');
    }
  }
...
export async function ingestWitnessSig(
  ...
  // Policy check
  if (policyHook) {
    const policyResult = await policyHook.check('witness.sig.ingest', { ns, org: sig.org, id: sig.id });
    if (!policyResult.allowed) {
      auditHook?.log('WITNESS_SIG_INGEST_DENIED', { ns, n: base.n, org: sig.org, id: sig.id, reason: policyResult.reason });
      throw new Error('policy_denied');
    }
  }
  ...
  if (existing) {
    ...
    const existingSig = witnessedCheckpoint.sigs.find(s => s.org === sig.org && s.id === sig.id);
    if (!existingSig) {
...
export async function verifyWitnessedCheckpoint(
  ...
  // Policy check
  if (policyHook) {
    const policyResult = await policyHook.check('witness.verify', { ns, n: wc.n, policy });
    if (!policyResult.allowed) {
      auditHook?.log('WITNESS_VERIFY_DENIED', { ns, n: wc.n, reason: policyResult.reason });
