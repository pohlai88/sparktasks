...
export async function setActiveSigner(ns: string, kid: string): Promise<void> {
  ...
  if (newSigner.status === 'REVOKED')
    throw new Error(`Cannot activate revoked signer ${kid}`);
...
export async function rotateSigner(
  ns: string,
  newSigner: Omit<SignerRecord, 'status' | 'createdAt'>,
  opts?: { overlapUntil?: string }
): Promise<void> {
