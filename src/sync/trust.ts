...
export async function addTrustedSigner(ns: string, pubB64u: string, actor?: string): Promise<void> {
  ...
  // Emit audit event
  await AuditApi.log('TRUST_SIGNER_ADDED', {
    namespace: ns,
...
export async function removeTrustedSigner(ns: string, pubB64u: string, actor?: string): Promise<void> {
  ...
  // Emit audit event
  await AuditApi.log('TRUST_SIGNER_REMOVED', {
    namespace: ns,
    signerPubKey: pubB64u,
    totalSigners: updated.length
  }, actor || 'system');
...
export async function auditPackAttested(ns: string, eventsHash: string, signer: string, actor?: string): Promise<void> {
  await AuditApi.log('PACK_ATTESTED', {
    namespace: ns,
    eventsHash,
    signerPubKey: signer,
...
export async function auditPackVerification(
  ...
  const eventType = result === 'success' ? 'PACK_VERIFY_OK' : 'PACK_VERIFY_FAIL';
