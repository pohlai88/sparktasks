...
const capKey = (ns: string, op: string, actorId: string, date: string) =>
  `policy:${ns}:cap:${op}:${actorId}:${date}`;
...
export async function savePolicies(
  ns: string,
  storage: StorageDriver,
  policies: PolicySetV1,
  actor: Actor
): Promise<void> {
  await MembershipApi.assertPermission(actor.id, 'TASK_WRITE');
  if (actor.role !== 'OWNER')
    throw new Error('Policy updates require OWNER role');
...
export async function checkPolicy(
  ...
  for (const rule of policies.rules) {
    ...
    if (
      rule.actorMinRole &&
      ROLE_LEVELS[ctx.actorRole] < ROLE_LEVELS[rule.actorMinRole]
    )
      continue;
    if (
      rule.targetMaxRole &&
      ctx.targetRole &&
      ROLE_LEVELS[ctx.targetRole] > ROLE_LEVELS[rule.targetMaxRole]
    )
      return 'deny';
    if (rule.nsAllow && !rule.nsAllow.includes(ctx.ns)) continue;
    if (
      rule.time &&
      !isInTimeWindow(ctx.nowISO, rule.time.start, rule.time.end)
    )
      continue;
...
export async function enforcePolicy(
  ...
  if (result === 'deny') {
    ...
    throw new Error(
      `POLICY_DENIED: Operation '${ctx.op}' denied for actor '${ctx.actorId}' in namespace '${ctx.ns}'`
    );
  }
  ...
  if (commitCap && result === 'allow') {
    ...
    if (policies) {
      for (const rule of policies.rules) {
        if (
          rule.perActorDailyCap !== undefined &&
...
const federationPolicies = new Map<string, FederationPolicyConfig>();

export function configureFederationPolicy(
  ns: string,
  config: FederationPolicyConfig
): void {
