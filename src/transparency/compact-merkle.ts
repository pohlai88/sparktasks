...
async function leafHash(leafBytes: Uint8Array): Promise<string> {
  return hash(
    LEAF_PREFIX,
    new Uint8Array(
      await crypto.subtle.digest('SHA-256', new Uint8Array(leafBytes))
    )
  );
}

async function nodeHash(left: string, right: string): Promise<string> {
  const l = new Uint8Array(fromB64u(left)),
    r = new Uint8Array(fromB64u(right));
...
export async function appendLeaf(
  ...
  return {
    index: state.n - 1,
    leafHashB64u,
    rootB64u: await computeRoot(state.frontier),
    n: state.n,
  };
}
...
export async function verifyProof(
  ...
  try {
    ...
    for (const sibling of proof.siblings) {
      currentHash =
        currentIndex & 1
          ? await nodeHash(sibling, currentHash)
          : await nodeHash(currentHash, sibling);
