...
import type { CreateRecoveryOverrideArgs, RecoveryOverrideEnvelope, RecoveryOverrideContentV1 } from './override.types';
...
export async function createRecoveryOverride(opts: CreateRecoveryOverrideArgs & { storage?: StorageDriver }): Promise<{ envelope: RecoveryOverrideEnvelope; id: string }> {
  const { ns, actorId, beneficiaryId, code, expiresAt, scope = "ALL", sign, storage } = opts;
  ...
  if (storage && actorRole) {
    await enforcePolicy(
      { ns, op: 'override.create', actorId, actorRole, targetRole: beneficiaryRole, nowISO: new Date().toISOString() },
  ...
  if (!allowedTargets.includes(beneficiaryRole)) {
    throw new Error(`Access denied: ${actorRole || 'none'} cannot create override for ${beneficiaryRole}`);
  }
  ...
  await AuditApi.log('RECOVERY_OVERRIDE_CREATED', {
    overrideId: id,
    beneficiaryId,
    actorRole: actorRole,
