...
export async function acceptRecoveryOverride(opts: AcceptRecoveryOverrideArgs & { storage?: StorageDriver }): Promise<{ imported: number; scope: "ALL" | "ACTIVE" }> {
  const { ns, envelope, code, keyring, beneficiaryId, membership, storage } = opts;
  ...
  if (storage && beneficiaryRole) {
    await enforcePolicy(
      { ns, op: 'override.accept', actorId: beneficiaryId, actorRole: beneficiaryRole, nowISO: new Date().toISOString() },
  ...
  if (content.ns !== ns) {
    throw new Error(`Namespace mismatch: expected '${ns}', got '${content.ns}'`);
  }
  ...
  await AuditApi.log('RECOVERY_OVERRIDE_USED', {
    ...
    timeToAccept: content.exp ? 
      new Date().getTime() - new Date(content.exp).getTime() + (7 * 24 * 60 * 60 * 1000) : undefined // Rough usage timing
  }, beneficiaryId);
