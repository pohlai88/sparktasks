...
import { Card, Button, IconButton, Input, Textarea, PriorityBadge, DueDateBadge, TagList, DESIGN_TOKENS } from './ui';
...
export function TaskCard({ 
      ...
      <Card elevation="lg" padding="default">
        <form role="form" aria-label="Edit task">
          ...
          <Textarea
            ...
            resize="vertical"
          />
          <div className={cn('flex justify-end space-x-3 pt-4 border-t', TOKENS.colors.states.default.border)}>
            <Button
              variant="secondary"
  ...
  return (
    <Card 
      variant="interactive"
      className={`group relative ${isFocused ? 'ring-2 ring-blue-500 ring-offset-2' : ''}`}
    >
      <article 
        ref={cardRef}
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <h3 className={cn(
            'font-semibold text-base leading-6 mb-2 transition-colors',
          ...
          {task.notes && (
            <p className={cn(
              ...
              task.status === 'DONE' ? TOKENS.colors.ui.text.muted : TOKENS.colors.ui.text.secondary
            )}>
              {task.notes}
            </p>
          )}
      ...
      {isFocused && (
        <div className={cn(
          'mt-3 pt-3 border-t font-medium',
          TOKENS.colors.states.default.border,
          TOKENS.typography.body.small,
          TOKENS.colors.ui.text.muted
        )}>
          j/k: navigate • m: move • space: toggle • s: snooze • e: edit • del: delete
        </div>
      )}
