...
- **ZIndex Orchestration**: Overlay components use ZIndexOrchestrator for conflict-free layering
...
```json
{
  "command-system": ["cmdk@^1.0.4", "framer-motion@^11.11.9"],
  "drag-drop": [
    "@dnd-kit/core@^6.1.0",
...
export const aiDocumentation = {
  generateStorybook:
    'AI creates comprehensive Storybook stories with all variants',
  generateExamples:
    'AI generates real-world usage examples with best practices',
...
export function CommandPalette({ className, surface = 'glass', ...props }) {
  // All styling through enhanced tokens
  const paletteClasses = cn(
    ENHANCED_DESIGN_TOKENS.surfaces[surface],
    ENHANCED_DESIGN_TOKENS.spacing.p4,
    className
  )

  // TokenGuard available for development-time validation if needed
  // const { validateProps } = useTokenGuard({ enforceTokens: true })
