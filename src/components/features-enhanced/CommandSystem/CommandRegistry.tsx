...
const CommandRegistryContext = React.createContext<UseCommandRegistryReturn | null>(null);
...
export function CommandRegistry({
  ...
  const saveHistory = React.useCallback((history: Command[]) => {
    if (!finalConfig.persistHistory) return;
  ...
  const registerCommands = React.useCallback((commands: Command[]) => {
  ...
  const registerGroup = React.useCallback((group: CommandGroup) => {
    setState(prev => {
  ...
  const registerShortcut = React.useCallback((keys: string[], action: () => void) => {
  ...
  const searchCommands = React.useCallback((query: string): Command[] => {
    const commands = [...state.commands.values()];

    if (!query.trim()) return commands;

    const lowerQuery = finalConfig.caseSensitive ? query : query.toLowerCase();
    ...
    return commands.filter(command => {
      ...
      const textToSearch = finalConfig.caseSensitive ? searchableText : searchableText.toLowerCase();
      return textToSearch.includes(lowerQuery);
    });
  }, [state.commands, finalConfig.caseSensitive]);

  const filterCommands = React.useCallback((filter: CommandFilter): Command[] => {
    const commands = [...state.commands.values()];
    ...
    return commands.filter(command => {
      ...
      if (filter.tags && filter.tags.length > 0) {
        ...
        const hasMatchingTag = filter.tags.some(tag => commandTags.includes(tag));
        if (!hasMatchingTag) return false;
      }

      // Enabled filter
      if (filter.enabled !== undefined && !!command.disabled === filter.enabled) {
        return false;
      }
  ...
  const executeCommand = React.useCallback(async (commandId: string): Promise<void> => {
    ...
    // Add to history
    const newHistory = [command, ...state.history.filter(h => h.id !== commandId)]
  ...
  const canExecuteCommand = React.useCallback((commandId: string): boolean => {
    const command = state.commands.get(commandId);
  ...
  }, [state.shortcuts, finalConfig.globalShortcuts, finalConfig.enableInInputs, finalConfig.preventDefaults]);
  ...
  return (
    <CommandRegistryContext.Provider value={contextValue}>
      <div className={cn('command-registry', className)}>
        {children}
      </div>
