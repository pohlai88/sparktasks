...
async function analyzeBundleSize(): Promise<BundleAnalysis> {
  ...
  try {
    ...
    const jsBundles = buildFiles.filter(
      file =>
        typeof file === 'string' &&
        file.endsWith('.js') &&
        !file.includes('.map')
...
async function analyzeRadixUsage(): Promise<
  Array<{
    primitive: string;
    usage: number;
    components: string[];
...
function generateRecommendations(analysis: BundleAnalysis): string[] {
  ...
  if (analysis.totalSize > 500_000) {
    ...
    recommendations.push(
      '⚠️ Bundle size is large (>500KB). Consider code splitting.'
    );
  }
  ...
  if (largeComponents.length > 0) {
    recommendations.push(
      `📏 Large components detected: ${largeComponents.map(c => c.name).join(', ')}`
    );
  }
...
${
  analysis.components
    ...
    .map(
      c =>
        `- **${c.name}**: ${formatBytes(c.size)} (${c.dependencies.length} deps)`
    )
...
${analysis.radixPrimitives
  ...
  .map(
    p =>
      `- **${p.primitive}**: Used in ${p.usage} components (${p.components.join(', ')})`
  )
  .join('\n')}
...
async function main() {
  try {
    ...
    console.log(
      `Container Components: ${analysis.components.filter(c => c.isContainer).length}`
    );
...
if (typeof process !== 'undefined' && process.argv && process.argv.length > 1) {
  ...
  if (
    scriptPath &&
    currentFile &&
    path.resolve(scriptPath) === path.resolve(currentFile)
  ) {
    main();
  }
}
