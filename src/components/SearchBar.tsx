...
export function SearchBar({ onTaskSelect, className = '' }: SearchBarProps) {
  ...
  const highlightText = (text: string, query: string) => {
    ...
    return parts.map((part, index) => 
      regex.test(part) ? (
        <mark key={index} className={DESIGN_TOKENS.recipes.searchResultHighlight}>{part}</mark>
      ) : part
    );
  };

  const activeDescendant = state.activeIndex >= 0 ? getOptionId(state.activeIndex) : undefined;
    ...
    <div className={`relative ${className}`}>
      ...
      <div className="relative" ref={inputRef}>
        <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
          <Search className={cn('size-5', DESIGN_TOKENS.colors.ui.text.muted)} />
        </div>
        <Input
          type="text"
          value={state.query}
        ...
        {state.query && (
          <Button
            ...
            onClick={() => {
              setState(prev => ({ ...prev, query: '', isOpen: false, results: [], activeIndex: -1 }));
      ...
      {state.isOpen && (
        ...
        >
          <Card className={cn(
            'absolute z-50 w-full mt-1 max-h-60 overflow-auto p-0',
            DESIGN_TOKENS.recipes.searchResults
          )}>
          {state.loading ? (
            <div className={cn(
              DESIGN_TOKENS.recipes.searchNoResults
            )} role="status">
              Searching...
            </div>
                ...
                <div
                  ...
                  className={cn(
                    ...
                    index === state.activeIndex
                      ? DESIGN_TOKENS.colors.status.active.bg + ' ' + DESIGN_TOKENS.colors.status.active.text
                      : DESIGN_TOKENS.colors.ui.text.primary + ' ' + DESIGN_TOKENS.colors.states.default.hover
                  )}
                  onClick={() => handleTaskSelect(task)}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex-1 min-w-0">
                      <p className={cn(DESIGN_TOKENS.typography.body.primary, 'font-medium truncate')}>
                        {highlightText(task.title, state.query)}
                      </p>
                      {task.notes && (
                        <p className={cn(DESIGN_TOKENS.typography.body.small, 'truncate mt-1')}>
                          {highlightText(task.notes, state.query)}
                        </p>
                      )}
                    </div>
                    <div className={cn('flex items-center ml-3', DESIGN_TOKENS.spacing.inlineTight)}>
                      {(task.status === 'TODAY' || task.status === 'LATER' || task.status === 'DONE') ? (
                  ...
                  {task.tags.length > 0 && (
                    <div className={cn('flex flex-wrap mt-2', DESIGN_TOKENS.spacing.inlineTight)}>
                      {task.tags.map(tag => (
