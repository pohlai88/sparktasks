...
export type DrawerVariant = 'default' | 'primary' | 'success' | 'warning' | 'danger';
...
const animationVariants: Record<DrawerPosition, { enter: string; exit: string }> = {
  left: {
...
const DrawerHeader = forwardRef<HTMLDivElement, DrawerHeaderProps>(
  ({ title, description, onClose, variant = 'default', dismissible = true }, ref) => {
  ...
  ({ actions, variant = 'default', position = 'right' }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={`
          ${DESIGN_TOKENS.layout.patterns.sectionFooter}
          ${variantStyles[variant]}
          sticky bottom-0 z-10 backdrop-blur-sm
          flex items-center justify-end gap-3
          ${position === 'top' || position === 'bottom' ? 'flex-row' : 'flex-col sm:flex-row'}
        `}
      >
        {actions.map((action, index) => (
          <button
            ...
            className={`
              ${DESIGN_TOKENS.recipe.button[action.variant || 'secondary']}
              ${DESIGN_TOKENS.motion.smooth}
...
const Drawer = forwardRef<HTMLDivElement, DrawerProps>(
  (
    ...
    const combinedRef = useCallback((node: HTMLDivElement | null) => {
      ...
      if (ref) {
        if (typeof ref === 'function') {
          ...
          // @ts-ignore - TypeScript ref typing issue
          ref.current = node;
        }
      }
    }, [ref]);
    ...
    const animationStyles = animateIn || animateOut ? 
      (open ? animationVariants[position].enter : animationVariants[position].exit) : 
      '';
    ...
    const drawerContent = (
      <>
        ...
        >
          ...
          {/* Content */}
          <div className={`
            ${DESIGN_TOKENS.layout.patterns.panelSection}
...
export const Sheet = forwardRef<HTMLDivElement, Omit<DrawerProps, 'modal'>>(
  (props, ref) => (
    <Drawer ref={ref} modal={true} backdrop={true} {...props} />
  )
);
 ...
 */
export interface SidebarProps extends Omit<DrawerProps, 'position' | 'modal' | 'backdrop'> {
  position?: 'left' | 'right';
  collapsible?: boolean;
  collapsed?: boolean;
 ...
 */
export const SlideOver = forwardRef<HTMLDivElement, Omit<DrawerProps, 'modal' | 'backdrop' | 'position'>>(
  (props, ref) => (
    <Drawer
      ref={ref}
      position="right"
 ...
 */
export const BottomSheet = forwardRef<HTMLDivElement, Omit<DrawerProps, 'position'>>(
  (props, ref) => (
    <Drawer
      ref={ref}
      position="bottom"
...
// Sheet Usage
<Sheet open={isOpen} onClose={() => setIsOpen(false)} title="User Details">
  <UserForm />
</Sheet>
