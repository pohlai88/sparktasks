 ...
 */
const getThumbnailInteractionClasses = (mode: ThumbnailMode, disabled: boolean): string => {
  if (disabled) {
...
export const Thumbnail = forwardRef<HTMLDivElement, ThumbnailProps>(({
  ...
  // Image loading state
  const { isLoading, hasError, handleLoad, handleError } = useThumbnailLoading(onLoad, onError);
  ...
  const loadingSkeleton = showSkeleton ? (
    <div className={`
      relative size-full animate-pulse overflow-hidden
      ${DESIGN_TOKENS.semantic.background.muted}
    `}>
