...
function getDefaultContent(variant: BannerVariant) {
  const contentMap = {
    ...
    maintenance: {
      ...
      description: 'System maintenance is scheduled for tonight at 2:00 AM UTC.'
    },
    promotion: {
      title: 'Special Offer',
...
const BannerIconComponent: React.FC<BannerIconComponentProps> = ({ variant, size, icon }) => {
  ...
  return (
    <div className={cn(sizeClasses.icon, variantClasses.icon, 'flex-shrink-0')}>
      {iconElement}
    </div>
  );
};
...
const BannerActions: React.FC<BannerActionsProps> = ({ actions, size }) => {
  ...
  return (
    <div className={cn('flex items-center', sizeClasses.actionSpacing)}>
...
const Banner = React.forwardRef<HTMLDivElement, BannerProps>(({
  ...
  React.useEffect(() => {
    if (persistenceKey && typeof window !== 'undefined') {
      const dismissed = localStorage.getItem(`banner-dismissed-${persistenceKey}`);
      if (dismissed) {
        setIsDismissed(true);
        setIsVisible(false);
      }
  ...
  return (
    <div
      ...
      {...props}
    >
      <BannerIconComponent variant={variant} size={size} {...(icon && { icon })} />
      
      <div className="min-w-0 flex-1">
        {displayTitle && (
          <h3 id="banner-title" className={cn(sizeClasses.title, variantClasses.title, 'font-semibold')}>
            {displayTitle}
          </h3>
        )}
        
        {displayDescription && (
          <p className={cn(sizeClasses.description, variantClasses.description, displayTitle && 'mt-1')}>
            {displayDescription}
          </p>
        )}
 ...
 */
const BannerInfo = React.forwardRef<HTMLDivElement, Omit<BannerProps, 'variant'>>(
 ...
 */
const BannerSuccess = React.forwardRef<HTMLDivElement, Omit<BannerProps, 'variant'>>(
 ...
 */
const BannerWarning = React.forwardRef<HTMLDivElement, Omit<BannerProps, 'variant'>>(
 ...
 */
const BannerError = React.forwardRef<HTMLDivElement, Omit<BannerProps, 'variant'>>(
 ...
 */
const BannerAnnouncement = React.forwardRef<HTMLDivElement, Omit<BannerProps, 'variant'>>(
 ...
 */
const BannerMaintenance = React.forwardRef<HTMLDivElement, Omit<BannerProps, 'variant'>>(
 ...
 */
const BannerPromotion = React.forwardRef<HTMLDivElement, Omit<BannerProps, 'variant'>>(
