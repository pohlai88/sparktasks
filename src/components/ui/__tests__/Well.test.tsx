...
describe('Well Component - Basic Rendering', () => {
  ...
  it('spreads additional props correctly', () => {
    ...
    expect(screen.getByTestId('test-well')).toHaveAttribute('aria-describedby', 'description-id');
  });
});
...
describe('Well Component - Variants', () => {
  const variants: Array<'default' | 'primary' | 'secondary' | 'success' | 'warning' | 'error' | 'info' | 'interactive' | 'elevated' | 'flat' | 'outlined'> = [
    'default',
    'primary', 
    'secondary',
    'success',
  ...
  it('defaults to default variant when no variant provided', () => {
    renderWell();
    expect(screen.getByTestId('test-well')).toHaveAttribute('data-variant', 'default');
  });
...
describe('Well Component - Padding', () => {
  const paddings: Array<'none' | 'tight' | 'normal' | 'loose'> = ['none', 'tight', 'normal', 'loose'];
  ...
  it('defaults to normal padding when no padding provided', () => {
    renderWell();
    expect(screen.getByTestId('test-well')).toHaveAttribute('data-padding', 'normal');
  });

  it('applies correct padding classes', () => {
    const paddingClasses: Record<'none' | 'tight' | 'normal' | 'loose', string> = {
      none: 'p-0',
      tight: 'p-2',
      normal: 'p-4',
      loose: 'p-6'
    ...
    paddings.forEach(padding => {
      const { rerender } = render(<Well padding={padding} data-testid="test-well" />);
      const well = screen.getByTestId('test-well');
...
describe('Well Component - States', () => {
  ...
  it('renders disabled state correctly', () => {
    ...
    expect(well).toHaveClass('opacity-50', 'cursor-not-allowed', 'pointer-events-none');
  });
...
describe('Well Component - Compound Components', () => {
  ...
  it('applies correct styling to compound components', () => {
    ...
    expect(header).toHaveClass('border-b', 'border-slate-200', 'dark:border-slate-700');
...
describe('Well Component - Accessibility', () => {
  it('supports aria-label', () => {
    ...
    expect(screen.getByTestId('test-well')).toHaveAttribute('aria-label', 'Well description');
  });

  it('supports aria-labelledby', () => {
    renderWell({ 'aria-labelledby': 'label-id' });
    expect(screen.getByTestId('test-well')).toHaveAttribute('aria-labelledby', 'label-id');
  });
  ...
  it('sets proper ARIA attributes for states', () => {
    ...
    expect(screen.getByTestId('test-well')).toHaveAttribute('aria-disabled', 'true');
    
    rerender(<Well loading data-testid="test-well" />);
    expect(screen.getByTestId('test-well')).toHaveAttribute('aria-busy', 'true');
  });
...
describe('Well Component - Edge Cases', () => {
  ...
  it('maintains performance with frequent re-renders', () => {
    ...
    for (let i = 0; i < 100; i++) {
      rerender(<Well variant={i % 2 === 0 ? 'success' : 'error'} data-testid="test-well" />);
    }
...
describe('Well Component - Performance', () => {
  it('renders efficiently with minimal re-renders', () => {
    ...
    const TestComponent = ({ variant }: { variant: 'default' | 'success' }) => {
      renderCount();
      return <Well variant={variant} data-testid="test-well">Content</Well>;
    };
