...
const StatusIcon: React.FC<{ status: BadgeStatus }> = React.memo(({ status }) => {
  const iconProps = {
    className: 'size-3',
    fill: 'currentColor',
    viewBox: '0 0 20 20',
...
// ===== LOADING SKELETON =====
const BadgeSkeleton: React.FC<{ size?: BadgeSize }> = React.memo(({ size = 'md' }) => (
  <span
...
const BadgeComponent = forwardRef<HTMLSpanElement, BadgeProps>(function Badge(
  ...
  const handleKeyDownWrapped = useCallback((event: React.KeyboardEvent<HTMLSpanElement>) => {
    ...
    if (isDismissible && (event.key === 'Delete' || event.key === 'Backspace')) {
      event.preventDefault();
      handleDismiss(event);
    }
  ...
  const handleKeyUpWrapped = useCallback((event: React.KeyboardEvent<HTMLSpanElement>) => {
  ...
  const badgeClassName = useMemo(() => 
    combineTokens(
      ...
      isInteractive && combineTokens(
        'cursor-pointer hover:opacity-80',
        DESIGN_TOKENS.focus?.onLight ?? 'focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500',
    ...
    >
      ...
      {isDismissible && (
        <button
          type="button"
          className={combineTokens(
            DESIGN_TOKENS.recipe.iconButtonDefault ?? 'inline-flex items-center justify-center rounded-full focus:outline-none',
            'ml-1 size-4',
            DESIGN_TOKENS.focus?.onLight ?? 'focus:ring-1 focus:ring-inset focus:ring-white/20',
            DESIGN_TOKENS.focus?.forcedColors ?? '',
