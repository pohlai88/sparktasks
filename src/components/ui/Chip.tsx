...
const StatusIcon: React.FC<{ status: ChipStatus; size: ChipSize }> = React.memo(({ status, size }) => {
...
// ===== LOADING SKELETON =====
const ChipSkeleton: React.FC<{ size?: ChipSize }> = React.memo(({ size = 'md' }) => (
  <span
...
const ChipComponent = forwardRef<HTMLSpanElement, ChipProps>(function Chip(
  ...
  return (
    ...
    >
      ...
      {hasCounter && (
        <span 
          className="ml-1 inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-gray-100 px-1.5 text-xs font-medium text-gray-800 dark:bg-gray-700 dark:text-gray-200"
