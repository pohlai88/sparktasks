 ...
 */
export interface VideoProps extends Omit<React.VideoHTMLAttributes<HTMLVideoElement>, 'src' | 'poster' | 'preload'> {
  // ===== CONTENT =====
 ...
 */
const getSizeClasses = (size: VideoSize, aspectRatio: VideoAspectRatio): string => {
...
export const Video = forwardRef<HTMLVideoElement, VideoProps>(({
  ...
  const aspectRatioClasses = useMemo(() => getAspectRatioClasses(aspectRatio), [aspectRatio]);
  const sizeClasses = useMemo(() => getSizeClasses(size, aspectRatio), [size, aspectRatio]);
  ...
  return (
    ...
    >
      ...
      {showLoading && !hasError && (
        <div className={`absolute inset-0 ${DESIGN_TOKENS.utils.overlay} ${DESIGN_TOKENS.utils.centerAll}`}>
          {renderLoadingState()}
        </div>
      )}
      ...
      {/* Caption */}
      {caption && !hasError && (
        <div className={`absolute inset-x-0 bottom-0 p-2 ${DESIGN_TOKENS.utils.overlay}`}>
