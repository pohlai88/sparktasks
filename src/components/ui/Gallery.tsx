 ...
 */
export interface GalleryProps extends Omit<HTMLAttributes<HTMLDivElement>, 'onSelect'> {
...
export const Gallery = forwardRef<HTMLDivElement, GalleryProps>(({
  ...
  const handleKeyDown = useCallback((event: React.KeyboardEvent, item: GalleryItem, index: number) => {
    ...
    return (
      ...
      >
        <div className={DESIGN_TOKENS.layout.patterns.flexCenterCol}>
          <div className={`${DESIGN_TOKENS.semantic.text.error} ${DESIGN_TOKENS.typography.heading.h4}`}>
            Gallery Error
          </div>
          <p className={`${DESIGN_TOKENS.semantic.text.muted} ${DESIGN_TOKENS.typography.body.base}`}>
            {error}
          </p>
        </div>
      </div>
  ...
  if (items.length === 0) {
    return (
      ...
      >
        <div className={DESIGN_TOKENS.layout.patterns.flexCenterCol}>
          <div className={`${DESIGN_TOKENS.semantic.text.muted} ${DESIGN_TOKENS.typography.heading.h4}`}>
            No Images
          </div>
          <p className={`${DESIGN_TOKENS.semantic.text.muted} ${DESIGN_TOKENS.typography.body.base}`}>
  ...
  return (
    <>
      ...
      <div
        ...
        {...props}
      >
        <div ref={containerRef} className={layout === 'masonry' ? '' : 'contents'}>
          {items.map((item, index) => {
            ...
            return (
              <button
                ...
                className={`
                  ${tokens.item.base}
                  ${selectable ? tokens.item.selectable : ''}
                  ${isSelected ? tokens.item.selected : ''}
                  ${layout === 'masonry' ? tokens.item.masonry : ''}
                  ${itemClassName}
                `}
                {...(selectable ? { 'aria-selected': isSelected, tabIndex: 0 } : {})}
                onClick={() => handleItemClick(item, index)}
                onKeyDown={(e) => handleKeyDown(e, item, index)}
              >
                <div className={`relative overflow-hidden ${layout === 'masonry' ? '' : 'aspect-square'} rounded-md`}>
                  <img
                ...
                {showCaptions && (item.caption || item.title) && (
                  <div className={tokens.caption.container}>
                    {item.title && (
                      <div className={tokens.caption.title}>
                        {item.title}
                      </div>
                    )}
                    {item.caption && (
...
export const GalleryItem = forwardRef<HTMLButtonElement, GalleryItemProps>(({
  ...
  return (
    <button
      ...
      className={`
        ${tokens.item.base}
        ${onItemClick ? tokens.item.selectable : ''}
        ${selected ? tokens.item.selected : ''}
        ${className}
      `}
