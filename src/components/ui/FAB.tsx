...
export type FABVariant = 'primary' | 'secondary' | 'accent' | 'success' | 'warning' | 'error';
export type FABPosition = 'bottom-right' | 'bottom-left' | 'top-right' | 'top-left' | 'bottom-center' | 'custom';
...
export function FAB({
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const shouldHide = currentScrollY > lastScrollY.current && currentScrollY > scrollThreshold;
      const shouldShow = currentScrollY < lastScrollY.current || currentScrollY <= scrollThreshold;
  ...
  const getVariantClasses = (): string => {
    const variantMap = {
      ...
      warning: 'bg-warning-600 hover:bg-warning-700 text-white shadow-lg hover:shadow-xl',
      error: 'bg-error-600 hover:bg-error-700 text-white shadow-lg hover:shadow-xl'
    };
  ...
  return (
    <>
      ...
      >
        {/* Loading Spinner */}
        {loading && (
          <div className={`${sizeClasses.icon} animate-spin rounded-full border-2 border-current border-t-transparent`} />
        )}
