...
const getFormatIcon = (format: DocumentFormat): React.ReactNode => {
  ...
  switch (format) {
    ...
    default:
      return <FileText {...iconProps} className="size-5 shrink-0 text-gray-600" />;
  }
};
...
const getStatusIndicator = (status: DocumentStatus): React.ReactNode => {
  ...
  switch (status) {
    ...
    case 'review':
      return <AlertCircle {...iconProps} className="size-4 shrink-0 text-yellow-500" />;
    case 'approved':
      return <CheckCircle2 {...iconProps} className="size-4 shrink-0 text-blue-500" />;
    case 'final':
      return <CheckCircle2 {...iconProps} className="size-4 shrink-0 text-green-500" />;
    case 'archived':
      return <Archive {...iconProps} className="size-4 shrink-0 text-gray-500" />;
    default:
      return null;
  }
};
...
const getDocumentClasses = (
  ...
  const interactiveClasses = onClick ? combineTokens(
...
const Document = forwardRef<HTMLDivElement, DocumentProps>(({
  ...
  const documentClasses = getDocumentClasses(size, disabled, loading, onClick, hasCustomBackground);
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            ...
            <div className={combineTokens(
              ...
              isHovered || size === 'detailed' ? 'opacity-100' : 'opacity-0',
          ...
          {size === 'detailed' && metadata?.tags && metadata.tags.length > 0 && (
      ...
      {size === 'detailed' && thumbnailUrl && (
        <div className="mt-3">
          <img
            ...
            className={combineTokens(
              'h-32 w-full object-cover border',
