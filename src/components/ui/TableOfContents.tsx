...
const extractHeadingsFromContent = (
  ...
  const headingSelectors = Array.from({ length: maxLevel - minLevel + 1 }, (_, i) => 
    `h${minLevel + i}`
  ).join(', ');
  
  const headingElements = container.querySelectorAll(headingSelectors) as NodeListOf<HTMLHeadingElement>;
...
const buildNestedStructure = (headings: TOCHeading[]): TOCHeading[] => {
  ...
  const hasExistingStructure = headings.some(heading => heading.children && heading.children.length > 0);
...
const getTOCClasses = (
  ...
  const variantClasses = {
    ...
    card: combineTokens(DESIGN_TOKENS.recipe.card.base, DESIGN_TOKENS.recipe.card.elevated),
  }[variant];
  
  const sizeClasses = {
    sm: combineTokens(DESIGN_TOKENS.typography.body.xs, DESIGN_TOKENS.spacing.sm),
    md: combineTokens(DESIGN_TOKENS.typography.body.small, DESIGN_TOKENS.spacing.md),
    lg: combineTokens(DESIGN_TOKENS.typography.body.medium, DESIGN_TOKENS.spacing.lg),
  }[size];
  
  const stickyClasses = sticky ? combineTokens(
  ...
  const scrollClasses = sticky ? combineTokens(
...
const getTOCItemClasses = (
  ...
  const baseClasses = 'toc-item flex items-start transition-all duration-200 ease-out relative group';
  ...
  const interactiveClasses = 'cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800/60 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 focus:ring-offset-slate-50 dark:focus:ring-offset-slate-800';
...
const TOCSkeleton: React.FC<{ size?: 'sm' | 'md' | 'lg' }> = ({ size = 'md' }) => {
  const itemHeight = {
    sm: 'h-5',
    md: 'h-6',
    lg: 'h-7',
  ...
  return (
    <div className="animate-pulse space-y-2">
      {Array.from({ length: 5 }, (_, i) => (
        <div key={i} className={combineTokens('rounded bg-slate-200 dark:bg-slate-700', itemHeight)} 
...
const TOCItem = forwardRef<HTMLLIElement, TOCItemProps>(({
  ...
  return (
    <li ref={ref} className={combineTokens(itemClasses, className)} {...props}>
        <div 
          className={combineTokens(DESIGN_TOKENS.recipe.listItemContent.base, 'cursor-pointer')}
          onClick={handleClick}
        ...
        {heading.children && heading.children.length > 0 && (
          <ul className={combineTokens(DESIGN_TOKENS.spacing.workspace.subtitleMargin, DESIGN_TOKENS.spacing.stackTight)}>
...
const TableOfContents = forwardRef<HTMLElement, TableOfContentsProps>(({
  ...
  const [currentHeading, setCurrentHeading] = useState<TOCHeading | null>(null);
  const [collapsed, setCollapsed] = useState(defaultCollapsed);
  const observerRef = useRef<IntersectionObserver | null>(null);
  const headingElementsRef = useRef<Map<string, HTMLElement>>(new Map());
  
  // Find heading by ID in nested structure
  const findHeadingById = React.useCallback((headingList: TOCHeading[], id: string): TOCHeading | null => {
  ...
  useEffect(() => {
    ...
    if (propHeadings) {
      ...
      flatHeadings = extractHeadingsFromContent(contentSelector, minLevel, maxLevel);
    ...
    observerRef.current = new IntersectionObserver(
      (entries) => {
        ...
        if (visibleEntries.length > 0) {
          ...
          const activeHeading = Array.from(headingElementsRef.current.entries())
            .find(([headingId]) => headingElementsRef.current.get(headingId) === activeElement)?.[0];
  ...
  }, [headings, highlightCurrent, scrollOffset, currentHeading, onCurrentHeadingChange, findHeadingById]);
  ...
  return (
    ...
    >
      ...
      {showTitle && (
        <div className={combineTokens(DESIGN_TOKENS.layout.flexBetween, DESIGN_TOKENS.spacing.workspace.headerMargin)}>
          <h3 className={DESIGN_TOKENS.typography.heading.h5}>{title}</h3>
          {collapsible && (
            <button
              type="button"
      ...
      {showProgress && !collapsed && (
        <div className={DESIGN_TOKENS.spacing.workspace.sectionMargin}>
          <div className={combineTokens('overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700', DESIGN_TOKENS.sizing.avatar.xs)}>
            <div 
              className="h-full bg-primary-500 transition-all duration-300 ease-out"
              style={{ 
                width: `${currentHeading ? 
                  ((headings.findIndex(h => h.id === currentHeading.id) + 1) / headings.length) * 100 
                  : 0}%` 
              }}
            />
          </div>
