...
export function setRevocationStorage(revocationStorage: RevocationStorage, ns = 'spark'): void {
...
async function loadSet(store: RevocationStorage, key: string): Promise<Set<string>> {
  const data = await store.getItem(key);
  return new Set(data ? JSON.parse(data) : []);
}

async function saveSet(store: RevocationStorage, key: string, set: Set<string>): Promise<void> {
 ...
 */
export async function revokeSigner(pubB64u: string, opts?: {
  ns?: string;
  actorId?: string;
  actorRole?: Role;
  ...
  if (opts?.storage && opts.actorId && opts.actorRole) {
    await enforcePolicy(
      { ns: opts.ns || namespace, op: 'signer.revoke', actorId: opts.actorId, actorRole: opts.actorRole, nowISO: new Date().toISOString() },
      opts.storage, { audit: true, commitCap: true }
    );
  }
