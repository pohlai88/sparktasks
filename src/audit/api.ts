...
function redactSensitive(ctx?: Record<string, unknown>): Record<string, unknown> | undefined {
  if (!ctx) return undefined;
  const redacted = { ...ctx };
  const sensitive = ['passcode', 'ctB64u', 'ivB64u', 'wrapped', 'privateKey', 'secret'];
...
export async function log(type: AuditType, ctx?: Record<string, unknown>, actor?: string): Promise<AuditEntry> {
...
export async function list(q?: Query): Promise<Page> {
  ...
  return { items, ...(filteredKeys.length > limit && { nextCursor: filteredKeys[limit] }) };
}

export async function exportAll(): Promise<{ items: AuditEntry[], valid: boolean }> {
  if (!storage || !namespace) {
    throw new Error('Audit not configured. Call configureAudit() first.');
  }
...
export async function verifyChain(items: AuditEntry[]): Promise<boolean> {
  ...
  for (const entry of items) {
    ...
    const computedHash = await hash((entry.prev || '') + canon(entryWithoutHash));
