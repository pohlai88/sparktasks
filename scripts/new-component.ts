...
const validFolders = ['ui', 'features', 'layout', 'data', 'demo'] as const;
if (!validFolders.includes(folderType as any)) {
  console.error(`❌ Invalid folder type. Use: ${validFolders.join(', ')}`);
  process.exit(1);
}

function createComponentFile(name: string, folder: string): string {
...
function createDemoFile(name: string, folder: string): string {
...
function updateIndexExports(name: string, folder: string): string {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.error('❌ Component creation failed:', (error as Error).message);
    process.exit(1);
  }
}
