...
- **BYOS (Bring Your Own Storage)**: Google Drive, Dropbox, or local file system _(planned)_
- **Local-First Architecture**: Offline-capable with real-time sync when connected _(partially implemented)_
- **End-to-End Encryption**: User-controlled keys, zero-trust security model _(foundations built)_
- **Performance-Optimized**: Sub-200ms search at scale, 60fps interactions _(target, not achieved)_
