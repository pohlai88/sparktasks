...
if (record.workspaceId !== namespace) {
  result.errors.push(`Cross-workspace replay blocked: ${record.workspaceId} != ${namespace}`);
  continue;
}
```
...
if (prevRole && ROLE_LEVELS[prevRole] > ROLE_LEVELS[record.role]) {
  console.warn(`Role downgrade: ${record.user} ${prevRole}→${record.role} by ${record.issuer.pubB64u} at ${record.ts}`);
}
```
...
if (skewMs > skewTolerance) 
  console.warn(`Clock skew detected: record ${id} offset ${Math.round(skewMs/1000)}s (tolerance: ${Math.round(skewTolerance/1000)}s)`);
```
