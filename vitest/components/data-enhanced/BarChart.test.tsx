...
describe('BarChart', () => {
  describe('Basic Rendering', () => {
    it('renders chart with data', () => {
      ...
      expect(chartContainer).toHaveAttribute(
        'aria-label',
        'Bar chart showing revenue, profit data'
      );
    });

    it('renders empty state when no data provided', () => {
      render(<BarChart data={[]} keys={['revenue']} indexBy='month' />);

      expect(screen.getByText('No data available')).toBeInTheDocument();
      expect(
        screen.getByText('Add data to display the chart')
      ).toBeInTheDocument();
    });
  ...
  describe('Chart Configuration', () => {
    it('applies vertical layout by default', () => {
      render(<BarChart data={mockData} keys={['revenue']} indexBy='month' />);
